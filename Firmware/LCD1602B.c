...
sbit	LCD_ENA	= P1^6;	//Pin 6
sbit	LCD_RW	= P1^5;	//Pin 5	//LCD_RS   R/W   DB7--DB0        FOUNCTION
sbit	LCD_RS	= P2^1;	//Pin 4	//	0		0	  INPUT      write the command to LCD model
...
void ClearLine(u8 hang)
{
	...
	Write_AC(hang_mod,1);	//set the AC pointer
	if((hang==1)||(hang==2))
		for(i=0; i<LineLenth; i++)	Write_DIS_Data(' ');
 	if((hang==3)||(hang==3))
		for(i=20; i<LineLenth+20; i++)	Write_DIS_Data(' ');
}
