...
#define NUMBER 20
#define SIZE 5
static unsigned char data status;
//存放五字节数据的数组
static unsigned char data value_array[SIZE];
/*可在其他的文件引用温湿度值,实际是温度的整数的 10 倍
如 dht11 读回的温度是 26,则 temp_value = 260, 湿度同理*/
int data temp_value, humi_value;
...
{
 unsigned char data count, value = 0, i;
 status = OK; //设定标志为正常状态
 for(i = 8; i > 0; i--)
 {
 //高位在先
...
{
 unsigned char data i = 0, check_value = 0,count = 0;
 EA = 0;
